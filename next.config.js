...
const nextConfig = {
  ...
  // `next lint` is wired up (.eslintrc → next/core-web-vitals). The codebase is
  // now free of lint *errors* (only exhaustive-deps warnings remain, which do
  // not fail the build), so lint is enforced at build time.
  eslint: {
